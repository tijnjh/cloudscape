  ...
  return (
    <>
      ...
      <div className="flex flex-wrap gap-2">
        {themeModes.map(themeModeOption => (
          <Button
            key={themeModeOption}
            variant={themeMode === themeModeOption ? 'primary' : 'secondary'}
            onClick={() => setThemeMode(themeModeOption)}
      ...
      <div className="flex flex-wrap gap-2">
        <Swatch
          ...
          onClick={() => {
            setIsBlackAccent(!isBlackAccent)
          }}
        />
...
function Swatch({
  ...
  return (
    <Button style={style} onClick={onClick} variant={isSelected ? 'primary' : 'secondary'}>
      <div className="size-3 rounded-full bg-(--swatch-color-light) outline-2 outline-base-300-700 dark:bg-(--swatch-color-dark)" />
      {color}
    </Button>
  )
}
